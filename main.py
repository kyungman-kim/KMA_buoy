...
def parse_response(text):
    ...
    for line in text.splitlines():
        ...
        if not debug_printed:
            print(f"DEBUG  columns({len(parts)}): {parts}")
            debug_printed = True
        if len(parts) < 9:
            continue
        try:
            tm = datetime.strptime(parts[1], "%Y%m%d%H%M").replace(tzinfo=KST)
...
def make_plot(rows, out_path):
    ...
    axes[0].set_ylabel("풍속 (m/s)"); axes[0].grid(True, alpha=0.3)

    axes[1].plot(times, wd, marker="o", color="orange")
    axes[1].set_ylabel("풍향 (°)"); axes[1].set_ylim(0, 360); axes[1].grid(True, alpha=0.3)

    axes[2].plot(times, wh, marker="o", color="navy")
    axes[2].set_ylabel("유의파고 (m)"); axes[2].grid(True, alpha=0.3)

    axes[2].xaxis.set_major_formatter(mdates.DateFormatter("%m/%d\n%H시"))
    fig.autofmt_xdate()
    fig.suptitle(f"포항 해양기상부이 · {datetime.now(KST):%Y-%m-%d %H:%M KST}", fontsize=14)
    fig.tight_layout()
